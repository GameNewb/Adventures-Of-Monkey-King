...
image_speed = .8;
sprite_index = spr_player_thrust_attack;
...
if(image_index >= 2 && attacked == false)
{
    ...
    audio_play_sound_on(audio_em, snd_swing, false, 3);
    
    //Create damage instance
    if(image_xscale >= 0) //Facing right
    {
        var damage = instance_create(x, y, obj_player_thrust_damage);
        damage.creator = id;
        attacked = true;
    }
    else if(image_xscale = -1) //Facing left
    {
        var damage = instance_create(x-52, y, obj_player_thrust_damage); //Equal to the id of the damage we created
